if (isDedicated || isServer) then {
	...
	clearMagazineCargoGlobal crate04;
	clearWeaponCargoGlobal crate04;
	clearItemCargoGlobal crate04;
	clearBackpackCargoGlobal crate04;
	clearMagazineCargoGlobal crate05;
	clearWeaponCargoGlobal crate05;
	clearItemCargoGlobal crate05;
	clearBackpackCargoGlobal crate05;
	clearMagazineCargoGlobal crate06;
	clearWeaponCargoGlobal crate06;
	clearItemCargoGlobal crate06;
	clearBackpackCargoGlobal crate06;
	...
	_cx = getMarkerPos (zones select SAFEZONE) select 0;
	_cy = getMarkerPos (zones select SAFEZONE) select 1;
	_xr = -175 + floor(random 351);
	_yr = -175 + floor(random 351);
	...
	while {true} do {
		...
		_h4 = [crate04, "crate04m"] spawn resetCrateContent;
		_h5 = [crate05, "crate05m"] spawn resetCrateContent;
		_h6 = [crate06, "crate06m"] spawn resetCrateContent;
		
		sleep 5;
		WaitUntil { ((((position crate01) select 2) < 4) || (((position crate02) select 2) < 4) || (((position crate03) select 2) < 4) || (((position crate04) select 2) < 4) || (((position crate05) select 2) < 4) || (((position crate06) select 2) < 4))};
		...
		_g1 = [crate01, 0] spawn explodeCrate;
		_g2 = [crate02, 20] spawn explodeCrate;
		_g3 = [crate03, 40] spawn explodeCrate;
		_g4 = [crate03, 60] spawn explodeCrate;
		_g5 = [crate03, 80] spawn explodeCrate;
		_g6 = [crate03, 100] spawn explodeCrate;
	};
};